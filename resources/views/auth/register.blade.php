<x-layouts.auth title="Create Account">
    <div>
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Create your account') }}</h2>
        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">{{ __('Start building your second brain today') }}</p>
    </div>
    ...
    <div class="relative my-6">
        ...
        <div class="relative flex justify-center text-sm"><span class="px-4 bg-surface-50 dark:bg-surface-950 text-gray-500">{{ __('or continue with email') }}</span></div>
    </div>
    ...
    <form method="POST" action="{{ route('register') }}" class="space-y-5" x-data="{ showPassword: false }">
        ...
        <div class="flex items-start">
            ...
            <label for="terms" class="ml-2 text-sm text-gray-600 dark:text-gray-400">
                {{ __('I agree to the') }} <a href="#" class="text-primary-600 hover:text-primary-500 font-medium">{{ __('Terms of Service') }}</a> {{ __('and') }} <a href="#" class="text-primary-600 hover:text-primary-500 font-medium">{{ __('Privacy Policy') }}</a>
            </label>
        </div>
        @error('terms')
    ...
    <p class="mt-6 text-center text-sm text-gray-600 dark:text-gray-400">
        ...
        <a href="{{ route('login') }}" class="font-semibold text-primary-600 hover:text-primary-500">{{ __('Sign In') }}</a>
    </p>
</x-layouts.auth>
