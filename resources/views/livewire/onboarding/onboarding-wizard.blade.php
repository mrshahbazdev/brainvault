    ...
    <div class="w-full max-w-2xl">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between mb-2">
                <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('Step') }} {{ $step }} {{ __('of') }} {{ $totalSteps }}</span>
                <button wire:click="skip" class="text-sm text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors">{{ __('Skip setup') }}</button>
            </div>
        ...
        <div class="bg-white dark:bg-surface-900 rounded-3xl border border-gray-200 dark:border-gray-800 shadow-xl p-8">
            ...
            @if($step === 1)
                <div class="text-center mb-8">
                    ...
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Welcome to BrainVault!') }}</h2>
                    <p class="text-gray-500 mt-2">{{ __("Let's set up your profile") }}</p>
                </div>

                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Your Name') }}</label>
                        <input wire:model="name" type="text" class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-surface-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent" placeholder="{{ __('Enter your name') }}">
            ...
            @if($step === 2)
                <div class="text-center mb-8">
                    ...
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Choose Your Theme') }}</h2>
            ...
            @if($step === 3)
                <div class="text-center mb-8">
                    ...
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Your Interests') }}</h2>
                    <p class="text-gray-500 mt-2">{{ __('Help us personalize your experience') }}</p>
                </div>
            ...
            @if($step === 4)
                <div class="text-center mb-8">
                    ...
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Install Chrome Extension') }}</h2>
                    <p class="text-gray-500 mt-2">{{ __('Save bookmarks from any webpage with one click') }}</p>
                </div>
                ...
                <div class="bg-gray-50 dark:bg-surface-800 rounded-2xl p-6 text-center">
                    <div class="space-y-4">
                        <div class="flex items-center gap-3 text-left">
                            ...
                            <p class="text-sm text-gray-700 dark:text-gray-300">{{ __('Click the button below to install') }}</p>
                        </div>
                        <div class="flex items-center gap-3 text-left">
                            <div class="w-8 h-8 bg-primary-100 dark:bg-primary-900/30 rounded-lg flex items-center justify-center flex-shrink-0">
                                <span class="text-primary-600 font-bold text-sm">2</span>
                            </div>
                            <p class="text-sm text-gray-700 dark:text-gray-300">{{ __('Pin the extension to your toolbar') }}</p>
                        </div>
                        <div class="flex items-center gap-3 text-left">
                            <div class="w-8 h-8 bg-primary-100 dark:bg-primary-900/30 rounded-lg flex items-center justify-center flex-shrink-0">
                                <span class="text-primary-600 font-bold text-sm">3</span>
                            </div>
                            <p class="text-sm text-gray-700 dark:text-gray-300">{{ __('Start saving bookmarks from any page!') }}</p>
                        </div>
                    </div>

                    <button class="mt-6 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl shadow-lg transition-all">
                        {{ __('Install Chrome Extension') }}
                    </button>
                    <p class="text-xs text-gray-400 mt-2">{{ __('You can always install it later from Settings') }}</p>
                </div>
            @endif
