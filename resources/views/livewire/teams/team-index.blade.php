<div>
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Teams') }}</h1>
            <p class="text-sm text-gray-500 mt-1">{{ __('Collaborate with others on shared knowledge') }}</p>
        </div>
                ...
                <div class="bg-white dark:bg-surface-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5">
                    <div class="flex items-start justify-between mb-3">
                        ...
                        <div class="flex items-center gap-1">
                            ...
                            <button wire:click="deleteTeam({{ $team->id }})" wire:confirm="{{ __('Delete this team?') }}" class="p-1.5 text-gray-400 hover:text-red-600 rounded-lg hover:bg-gray-100 dark:hover:bg-surface-800 transition-colors">
                    ...
                    <div class="flex items-center gap-2 mt-3">
                        ...
                        <span class="text-xs text-gray-500">{{ $team->members->count() }} {{ Str::plural(__('member'), $team->members->count()) }}</span>
                    </div>
                </div>
            @endforeach
        </div>
    @else
        <div class="bg-white dark:bg-surface-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-8 text-center">
            ...
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-1">{{ __('Create your first team') }}</h2>
            <p class="text-sm text-gray-500 mb-4">{{ __('Invite colleagues to collaborate on shared bookmarks and notes.') }}</p>
    ...
    @if($showCreateModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" wire:click.self="$set('showCreateModal', false)">
            <div class="bg-white dark:bg-surface-900 rounded-2xl shadow-xl w-full max-w-md p-6 mx-4">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('Create Team') }}</h2>
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Team Name') }}</label>
                        <input wire:model="newName" type="text" placeholder="{{ __('Engineering Team') }}" class="w-full px-4 py-2.5 bg-gray-100 dark:bg-surface-800 border-0 rounded-xl text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500">
                    </div>
                    <div>
                ...
                <div class="flex items-center justify-end gap-3 mt-6">
                    <button wire:click="$set('showCreateModal', false)" class="px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-surface-800 rounded-xl transition-colors">{{ __('Cancel') }}</button>
                    <button wire:click="createTeam" class="px-6 py-2.5 bg-primary-600 hover:bg-primary-700 text-white text-sm font-semibold rounded-xl transition-colors">{{ __('Create') }}</button>
                </div>
            </div>
        </div>
    @endif
    ...
    @if($showInviteModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" wire:click.self="$set('showInviteModal', false)">
            <div class="bg-white dark:bg-surface-900 rounded-2xl shadow-xl w-full max-w-md p-6 mx-4">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('Invite Member') }}</h2>
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Email') }}</label>
                        <input wire:model="inviteEmail" type="email" placeholder="{{ __('colleague@example.com') }}" class="w-full px-4 py-2.5 bg-gray-100 dark:bg-surface-800 border-0 rounded-xl text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500">
                    </div>
                    <div>
                        ...
                        <select wire:model="inviteRole" class="w-full px-4 py-2.5 bg-gray-100 dark:bg-surface-800 border-0 rounded-xl text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500">
                            <option value="member">{{ __('Member - can view') }}</option>
                            <option value="editor">{{ __('Editor - can add & edit') }}</option>
                            <option value="admin">{{ __('Admin - full access') }}</option>
                        </select>
                    </div>
                </div>
                <div class="flex items-center justify-end gap-3 mt-6">
                    <button wire:click="$set('showInviteModal', false)" class="px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-surface-800 rounded-xl transition-colors">{{ __('Cancel') }}</button>
                    <button wire:click="sendInvite" class="px-6 py-2.5 bg-primary-600 hover:bg-primary-700 text-white text-sm font-semibold rounded-xl transition-colors">{{ __('Send Invite') }}</button>
                </div>
            </div>
        </div>
    @endif
