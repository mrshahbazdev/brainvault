<div>
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Research Projects</h1>
            <p class="text-sm text-gray-500 mt-1">Organize your research with Kanban boards</p>
        </div>
    ...
    @forelse($projects as $project)
        <div class="mb-8">
            <div class="flex items-center justify-between mb-4">
                ...
                <button wire:click="deleteProject({{ $project->id }})" wire:confirm="Delete this project and all tasks?" class="text-gray-400 hover:text-red-600 transition-colors">
    ...
    @empty
        <div class="bg-white dark:bg-surface-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-8 text-center">
            ...
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-1">Start Researching</h2>
            <p class="text-sm text-gray-500 mb-4">Create a project to organize tasks and link bookmarks to your research.</p>
    ...
    @if($showProjectModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" wire:click.self="$set('showProjectModal', false)">
            <div class="bg-white dark:bg-surface-900 rounded-2xl shadow-xl w-full max-w-md p-6 mx-4">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">New Research Project</h2>
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Project Name</label>
                        <input wire:model="newProjectName" type="text" placeholder="e.g., Machine Learning Survey" class="w-full px-4 py-2.5 bg-gray-100 dark:bg-surface-800 border-0 rounded-xl text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500">
                    </div>
                    <div>
                ...
                <div class="flex items-center justify-end gap-3 mt-6">
                    <button wire:click="$set('showProjectModal', false)" class="px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-surface-800 rounded-xl transition-colors">Cancel</button>
                    <button wire:click="createProject" class="px-6 py-2.5 bg-primary-600 hover:bg-primary-700 text-white text-sm font-semibold rounded-xl transition-colors">Create</button>
                </div>
            </div>
        </div>
    @endif
    ...
    @if($showTaskModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" wire:click.self="$set('showTaskModal', false)">
            <div class="bg-white dark:bg-surface-900 rounded-2xl shadow-xl w-full max-w-md p-6 mx-4">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Add Task</h2>
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Task Title</label>
                        <input wire:model="newTaskTitle" type="text" placeholder="What needs to be done?" class="w-full px-4 py-2.5 bg-gray-100 dark:bg-surface-800 border-0 rounded-xl text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Description</label>
                ...
                <div class="flex items-center justify-end gap-3 mt-6">
                    <button wire:click="$set('showTaskModal', false)" class="px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-surface-800 rounded-xl transition-colors">Cancel</button>
                    <button wire:click="createTask" class="px-6 py-2.5 bg-primary-600 hover:bg-primary-700 text-white text-sm font-semibold rounded-xl transition-colors">Add Task</button>
                </div>
            </div>
        </div>
    @endif
