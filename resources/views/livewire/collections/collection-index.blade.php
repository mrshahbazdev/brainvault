<div>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Organize your bookmarks into folders</p>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                ...
                <div class="flex items-start justify-between mb-3">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        ...
                        <div x-show="open" @click.away="open = false" class="absolute right-0 mt-1 w-36 bg-white dark:bg-surface-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 py-1 z-10">
                            <button wire:click="editCollection({{ $collection->id }})" @click="open = false" class="w-full text-left px-3 py-1.5 text-xs text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-surface-700">Edit</button>
                            <button wire:click="openCreateModal({{ $collection->id }})" @click="open = false" class="w-full text-left px-3 py-1.5 text-xs text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-surface-700">Add Sub-folder</button>
                            <button wire:click="deleteCollection({{ $collection->id }})" wire:confirm="Delete collection?" @click="open = false" class="w-full text-left px-3 py-1.5 text-xs text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20">Delete</button>
                        </div>
                    </div>
                </div>
                <a href="{{ route('bookmarks.index', ['collection' => $collection->id]) }}" class="block">
                    ...
                    <div class="flex items-center gap-3 text-xs text-gray-400">
                        ...
                        @if($collection->children->count() > 0)
                            <span>{{ $collection->children->count() }} sub-folders</span>
                        @endif
                    </div>
                </a>
                ...
                @if($collection->children->count() > 0)
                    <div class="mt-3 pt-3 border-t border-gray-100 dark:border-gray-800 space-y-1">
                        ...
                        @if($collection->children->count() > 3)
                            <span class="text-xs text-gray-400 pl-6">+{{ $collection->children->count() - 3 }} more</span>
                        @endif
                    </div>
                @endif
            </div>
        @empty
            <div class="col-span-full py-16 text-center">
                ...
                <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-white">No collections yet</h3>
                <p class="mt-1 text-sm text-gray-500">Create your first collection to organize bookmarks.</p>
            </div>
        @endforelse
    </div>
    ...
    @if($showCreateModal || $showEditModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative bg-white dark:bg-surface-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-800 w-full max-w-md p-6">
                <h2 class="text-lg font-bold text-gray-900 dark:text-white mb-4">{{ $showEditModal ? 'Edit' : 'New' }} Collection</h2>
                <form wire:submit="{{ $showEditModal ? 'updateCollection' : 'createCollection' }}">
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Name</label>
                            <input wire:model="name" type="text" required placeholder="My Collection"
                        ...
                        <div>
                            ...
                            <textarea wire:model="description" rows="2" placeholder="What's this collection about?"
