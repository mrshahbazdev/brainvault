<div>
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">{{ Auth::user()->bookmarks()->count() }} {{ __('total bookmarks') }}</p>
        </div>
    ...
    <div class="bg-white dark:bg-surface-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-4 mb-6">
        ...
        @if(count($selected) > 0)
            <div class="flex items-center gap-3 mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                ...
                <button wire:click="bulkDelete" wire:confirm="{{ __('Delete selected bookmarks?') }}" class="px-3 py-1.5 text-xs font-medium bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-400 rounded-lg hover:bg-red-200 dark:hover:bg-red-900/30 transition-colors">{{ __('Delete') }}</button>
            </div>
        @endif
    </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                            ...
                            <div class="flex items-center gap-1">
                                ...
                                <div class="relative" x-data="{ open: false }">
                                    ...
                                    <div x-show="open" @click.away="open = false" class="absolute right-0 mt-1 w-36 bg-white dark:bg-surface-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 py-1 z-10">
                                        ...
                                        <button wire:click="deleteBookmark({{ $bookmark->id }})" wire:confirm="{{ __('Delete this bookmark?') }}" @click="open = false" class="w-full text-left px-3 py-1.5 text-xs text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20">
                                            {{ __('Delete') }}
                                        </button>
                                    </div>
                                </div>
            ...
            @empty
                <div class="col-span-full py-16 text-center">
                    ...
                    <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-white">{{ __('No bookmarks found') }}</h3>
        ...
        <div class="bg-white dark:bg-surface-900 rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden divide-y divide-gray-100 dark:divide-gray-800">
            @forelse($bookmarks as $bookmark)
                <div class="flex items-center gap-4 px-4 py-3 hover:bg-gray-50 dark:hover:bg-surface-800 transition-colors group" wire:key="bookmark-list-{{ $bookmark->id }}">
                    ...
                    <button wire:click="deleteBookmark({{ $bookmark->id }})" wire:confirm="{{ __('Delete') }}?" class="p-1 text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
