...
<div class="max-w-3xl mx-auto">
    ...
    <div class="flex items-center gap-1 bg-gray-100 dark:bg-surface-800 rounded-xl p-1 mb-8 w-fit">
        <a href="{{ route('settings.profile') }}" class="px-4 py-2 text-sm font-medium rounded-lg text-gray-500 hover:text-gray-700 dark:hover:text-gray-300">Profile</a>
        <a href="{{ route('settings.tokens') }}" class="px-4 py-2 text-sm font-medium rounded-lg bg-white dark:bg-surface-700 text-gray-900 dark:text-white shadow-sm">API Tokens</a>
    </div>
    ...
    @if(session('token'))
        <div class="mb-6 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl">
            <p class="text-sm font-medium text-amber-800 dark:text-amber-300 mb-2">Your new API token (copy now - it won't be shown again):</p>
            <div class="flex items-center gap-2">
                ...
                <button onclick="navigator.clipboard.writeText('{{ session('token') }}')" class="px-3 py-2 bg-primary-600 text-white text-xs font-medium rounded-lg hover:bg-primary-700 transition-colors">Copy</button>
            </div>
        </div>
    @endif
    ...
    <div class="bg-white dark:bg-surface-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6 mb-6">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">Create API Token</h2>
        <p class="text-sm text-gray-500 mb-4">API tokens allow the Chrome extension and third-party apps to access BrainVault on your behalf.</p>
    ...
    <div class="bg-white dark:bg-surface-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Active Tokens</h2>
        @if($tokens->count() > 0)
            <div class="space-y-3">
                @foreach($tokens as $token)
                    <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-surface-800 rounded-xl">
                        <div>
                            ...
                            <p class="text-xs text-gray-500">Created {{ $token->created_at->diffForHumans() }} &middot; Last used {{ $token->last_used_at?->diffForHumans() ?? 'Never' }}</p>
                        </div>
        ...
        @else
            <p class="text-sm text-gray-500 text-center py-4">No API tokens yet. Create one to connect the Chrome extension.</p>
        @endif
    </div>
</div>
@endsection
